...
export function PrecisionSketchTools() {
  ...
  let px = 0,
    ...
    valid = true
  try {
    const a = quantity(x, 'mm'),
      b = quantity(y, polar ? '°' : 'mm')
    px = polar ? a * Math.cos((b * Math.PI) / 180) : a
    py = polar ? a * Math.sin((b * Math.PI) / 180) : b
  } catch {
    valid = false
  }
    ...
    <section className="section precision-sketch">
        ...
        onSubmit={(e) => {
          ...
          if (valid && drawing)
            window.dispatchEvent(
              new CustomEvent('okc:coordinate', { detail: { x: px, y: py, relative } }),
            )
        }}
      >
      ...
      <div className="precision-presets">
        {actions
          ...
          .map((a) => (
            <button key={a.id} disabled={!valid} onClick={() => chooseSketchAction(a)}>
              {
                (
                  {
                    'add-polygon': 'Polygon at X/Y',
                    'add-slot': 'Slot at X/Y',
                    'linear-pattern': 'Linear pattern',
                    'circular-pattern': 'Circular pattern',
                    'mirror-vertical': 'Mirror X',
                    'mirror-horizontal': 'Mirror Y',
      ...
      <button
        ...
        onClick={() =>
          state.setSketchSelection(
            sketch.sketch.entities.map((e) => ({ kind: 'entity' as const, id: e.id })),
          )
        }
      >
