...
export function WorkspaceWelcome({ onCatalogue }: { onCatalogue: () => void }) {
  ...
  return (
    <div className="workspace-welcome">
      ...
      <p>
        Start with a shape, draw your own,
        <br />
        or build around a real component.
      </p>
      <div className="start-cards">
        <button onClick={() => chooseAction(box)}>
          <span>▱</span>
          <strong>Create a solid</strong>
          <small>Box, cylinder or custom shape</small>
          <b>01 →</b>
        </button>
        <button onClick={() => chooseAction(sketch)}>
          <span>▧</span>
          <strong>Draw a sketch</strong>
          <small>Exact dimensions, your outline</small>
          <b>02 →</b>
        </button>
        <button onClick={onCatalogue}>
          <span>⊞</span>
          <strong>Add hardware</strong>
          <small>Boards, controls and fasteners</small>
          <b>03 →</b>
        </button>
      </div>
