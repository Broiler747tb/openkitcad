...
export function selectedObjectActions() {
  ...
  const selection =
    state.selection.kind === 'feature' && state.selection.bodyId
      ? { kind: 'body' as const, id: state.selection.bodyId }
      : state.selection
  ...
  return objectActions(
    selection,
    face?.normal ? { bodyId: face.bodyId, point: face.point, normal: face.normal } : null,
  )
}
...
export function extrusionAction(revolve = false): ObjectAction | null {
  ...
  const active =
    ...
    (state.selection.kind === 'feature'
      ? { bodyId: state.selection.bodyId!, featureId: state.selection.id! }
      : null)
  const body = state.doc.bodies.find((b) => b.id === active?.bodyId)
  const feature = body?.features.find((f) => f.id === active?.featureId)
  if (!body || feature?.kind !== 'sketch' || !sketchLoopSummary(feature.sketch).closedLoops)
    return null
