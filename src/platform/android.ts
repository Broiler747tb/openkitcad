...
export const usePenMode = create<{
  fingerMode: 'pan' | 'orbit'
  setFingerMode: (mode: 'pan' | 'orbit') => void
}>((set) => ({
  fingerMode: 'pan',
  setFingerMode: (fingerMode) => set({ fingerMode }),
}))
...
export function androidDownload(blob: Blob, filename: string): boolean {
  ...
  if (blob.size > 64 * 1024 * 1024) {
    alert('Android export limit is 64 MB. Export fewer objects.')
    return true
  }
  ...
  reader.onload = () => {
    ...
    if (data === undefined) {
      alert('Could not prepare export.')
      return
    }
