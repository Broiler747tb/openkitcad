...
export function runPrecisionTest() {
  ...
  check(
    'alignment still rounds free axis',
    snap.align === 'horizontal' && snap.point[0] === 10 && snap.point[1] === 0,
  )
  check(
    'NaN settings fall back',
    normalizePreferences({ gridStep: NaN }).gridStep === DEFAULT_PREFERENCES.gridStep,
  )
  ...
  check(
    'trim action includes clicked position',
    result?.kind === 'trim' && result.at[0] === 15 && result.at[1] === 10,
  )
  return results
}
