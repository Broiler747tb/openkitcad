...
export function runPowerTest() {
  ...
  const check = (name: string, pass: boolean) =>
    results.push({ name: 'Workshop: ' + name, pass, detail: pass ? 'passed' : 'unexpected result' })
  ...
  check(
    'reject executable expressions',
    throws(() => quantity('window.alert(1)')),
  )
  check('reject empty and overflow', throws(() => quantity('')) && throws(() => quantity('1e999')))
  let counter = 0
  const id = (prefix: string) => `${prefix}-${++counter}`
  const s = emptySketch(),
    rect = addExactShape(s, 'rectangle', [10, 20], 30, 10, 90, id)
  ...
  const original = JSON.stringify(s),
    constraintCount = s.constraints.length
  const copies = copyTransformed(
    s,
    rect,
    { dx: 40, dy: 0, angle: 0, scale: 1, centre: [0, 0], count: 3 },
    id,
  )
  ...
  check(
    ...
    JSON.stringify(s.points.slice(0, 5)) === JSON.stringify(prior.points),
  )
  check('array last step', near(s.points[13].x, 135))
  check(
    'reject fractional copy count',
    throws(() =>
      copyTransformed(
        s,
        rect,
        { dx: 0, dy: 0, angle: 0, scale: 1, centre: [0, 0], count: 1.5 },
        id,
      ),
    ),
  )
  check(
    ...
    throws(() =>
      copyTransformed(s, rect, { dx: 0, dy: 0, angle: 0, scale: -1, centre: [0, 0], count: 1 }, id),
    ),
  )
  const ring = addExactShape(s, 'ring', [0, 0], 20, 10, 0, id)
  check(
    'ring has two concentric circles',
    s.entities.filter((e) => ring.includes(e.id) && e.kind === 'circle').length === 2,
  )
  check('ring total circumference', near(geometryLength(s, ring), 30 * Math.PI))
  check(
    'reject inverted ring',
    throws(() => addExactShape(s, 'ring', [0, 0], 10, 20, 0, id)),
  )
  const scaled = copyTransformed(
    s,
    ring,
    { dx: 0, dy: 0, angle: 0, scale: 2, centre: [0, 0], count: 1 },
    id,
  )
  check('scale copies circle radii', near(geometryLength(s, scaled), 60 * Math.PI))
  setFixed(s, rect, false, id)
  check(
    'unfix keeps origin pinned',
    s.constraints.some((c) => c.kind === 'fix' && c.p === 'origin'),
  )
  setFixed(s, rect, true, id)
  setFixed(s, rect, true, id)
  check(
    'fix is idempotent',
    s.constraints.filter((c) => c.kind === 'fix' && c.p === s.points[1].id).length === 1,
  )
  deleteGeometry(s, copies)
  check('batch delete removes every selected edge', !s.entities.some((e) => copies.includes(e.id)))
  s.points.push({ id: 'unused', x: 1, y: 2 })
  cleanUnusedPoints(s)
  ...
  const rotated = copyTransformed(
    arc,
    ['arc'],
    { dx: 0, dy: 0, angle: 90, scale: 1, centre: [0, 0], count: 1 },
    id,
  )
  ...
  try {
    ...
    check(
      'Undo removes full operation',
      activeSketchFeature(useStore.getState())!.sketch.entities.length === 0,
    )
    useStore.getState().redo()
    check(
      'Redo restores full operation',
      activeSketchFeature(useStore.getState())!.sketch.entities.length === 2,
    )
    const stateBefore = JSON.stringify(useStore.getState().doc),
      pastBefore = useStore.getState().past.length
    check(
      'invalid operation rejects',
      throws(() =>
        powerActions()
          .find((a) => a.id === 'exact-ring')!
          .run(5, 20),
    ...
    check(
      'invalid operation leaves document and Undo untouched',
      stateBefore === JSON.stringify(useStore.getState().doc) &&
        pastBefore === useStore.getState().past.length,
    )
  } finally {
